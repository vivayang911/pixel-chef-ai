...
const ARCHETYPES: Archetype[] = [
  {
    emoji: '🔥',
    label: 'Fire Chef',
    traits: ['Crispy Lover', 'Fast Cooker', 'Bold Flavor', 'High Heat'],
  },
  {
    emoji: '🌿',
    label: 'Healthy Creator',
    traits: ['Balanced', 'Fresh', 'Light Taste', 'Veggie Fan'],
  },
  {
    emoji: '✨',
    label: 'Flavor Explorer',
    traits: ['Curious', 'Diverse', 'Creative', 'Aroma Hunter'],
  },
  {
    emoji: '🍖',
    label: 'Comfort Cook',
    traits: ['Rich', 'Hearty', 'Warm', 'Traditional'],
  },
  {
    emoji: '🧪',
    label: 'Kitchen Scientist',
    traits: ['Precise', 'Methodical', 'Balanced', 'Technique'],
  },
]
...
function pickArchetype(ingredients: Ingredient[], events: string[], score: TasteScore): Archetype {
  ...
  // Fire Chef: high heat + crispy + spicy
  if (fire >= 2 || events.includes('fireTooHigh')) {
    return ARCHETYPES[0]
  }
  // Healthy Creator: lots of vegetables, low calories
  if (green >= 3) {
    return ARCHETYPES[1]
  }
  // Flavor Explorer: diverse taste tags
  if (tagCounts.size >= 5 || events.includes('perfectFlavor')) {
    return ARCHETYPES[2]
  }
  // Kitchen Scientist: balanced stats
  const avg = (score.taste + score.creativity + score.nutrition) / 3
  if (avg >= 70 && Math.abs(score.taste - score.creativity) <= 10) {
    return ARCHETYPES[4]
  }
  // Comfort Cook
  if (umami >= 2) {
  ...
  return ARCHETYPES[1] // fallback to healthy
}
...
const SUGGESTIONS: { trigger: (ingredients: Ingredient[]) => boolean; dish: string; reason: string }[] = [
  {
    trigger: (ings) => ings.some((i) => i.id === 'pork-belly'),
    dish: 'Korean Spicy Chicken 🍗',
    reason: 'Similar crispy texture but lower calories — a healthier twist on your love for crunch!',
  },
  {
    trigger: (ings) => ings.some((i) => i.id === 'chicken'),
    dish: 'Teriyaki Salmon Bowl 🐟',
    reason: 'You enjoyed lean protein — try fish for a different kind of tender umami experience.',
  },
  {
    trigger: (ings) => ings.some((i) => i.id === 'fish'),
    dish: 'Garlic Butter Shrimp 🦐',
    reason: 'You like delicate seafood. Shrimp brings a satisfying snap and pairs perfectly with your garlic habit.',
  },
  {
    trigger: (ings) => ings.some((i) => i.id === 'chili'),
    dish: 'Sichuan Mapo Tofu 🌶',
    reason: 'Your spice tolerance is growing. Mapo Tofu is the next level of numbing heat.',
  },
  {
    trigger: () => true,
    dish: 'Mushroom Risotto 🍄',
    reason: 'A creamy, earthy dish that rewards patience — perfect for your evolving cooking style.',
  },
]
...
export function generatePersonalityReport(
  ...
  // Favorite ingredients: pick the top 3 by unique emoji
  const favoriteIngredients = [...new Set(ingredients.map((i) => i.emoji))].slice(0, 3)

  // Health score derived from nutrition + ingredient diversity
  const hasProtein = ingredients.some((i) => i.category === 'protein')
  const hasVeg = ingredients.some((i) => i.category === 'vegetable')
  const hasFlavor = ingredients.some((i) => i.category === 'flavor')
  ...
  return {
    personality: `${archetype.emoji} ${archetype.label}`,
